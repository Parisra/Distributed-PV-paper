...
def insert_electricity_distribution_grid(n, costs, distsolar=True, distbat=True , distribution_efficiency= 1, rooftop_factor=1):  ## addeed last two params
    # TODO pop_layout?
    # TODO options?
    ...
    n.madd("Link",
        nodes + " electricity distribution grid",  #DW for two link
        bus0=nodes,
        bus1=nodes + " low voltage",
        p_nom_extendable=True,
        p_min_pu=-1,    #-1 default, 0 for 2 link
        carrier="electricity distribution grid",
        efficiency=distribution_efficiency,   #0.9  for high loss, 0.5 for medium loss
        lifetime=costs.at['electricity distribution grid', 'lifetime'],
        capital_cost=costs.at['electricity distribution grid', 'fixed'] * cost_factor
    )
    ######
    #add two onw-way links for sensitivity
    #n.madd("Link",
    #    nodes + " electricity distribution grid UW",
    ...
    if options['no_distsolar'] or not (distsolar):
      potential = 0 #for no rooftop case
    else : 
      potential = 0.1 * 10 * pop_solar * rooftop_factor ## added for highrooftop case
    ...
    n.madd("Generator",
        ...
        capital_cost=costs.at['solar-rooftop', 'fixed'],  #*0.1 for cheaprooftop and *0.9 for VCL
        efficiency=n.generators.loc[solar, 'efficiency'],
        p_max_pu=n.generators_t.p_max_pu[solar],
        lifetime=costs.at['solar-rooftop', 'lifetime']
    )
     
    if not options['no_homebattery'] and (distbat): # no home batteries are addedif distbat is False
     ...
     n.madd("Store",
        ...
        e_nom_extendable=True,
        carrier="home battery",
        capital_cost=costs.at['home battery storage', 'fixed'],  ## *0.7 for cheapbattery
     ...
     n.madd("Link",
        ...
        capital_cost=costs.at['home battery inverter', 'fixed'],   ## *0.6875 for cheapbattery
        p_nom_extendable=True,
...
def add_storage_and_grids(n, costs):
    ...
    if not options["no_utility_battery"]:   ## addded for no battery option
     ...
     n.madd("Store",
        ...
        e_nom_extendable=True,
        carrier="battery",
        capital_cost=costs.at['battery storage', 'fixed'],   ## *0.7 for cheapbattery
        lifetime=costs.at['battery storage', 'lifetime']
     )
     ...
     n.madd("Link",
        ...
        efficiency=costs.at['battery inverter', 'efficiency']**0.5,
        capital_cost=costs.at['battery inverter', 'fixed'],   ## *0.6875 for cheapbattery
        p_nom_extendable=True,
        lifetime=costs.at['battery inverter', 'lifetime']
     )
...
def add_land_transport(n, costs, bev_add=1):
    # TODO options?
    ...
    if electric_share > 0 and options["v2g"] and bev_add:             ### added
    ...
    if electric_share > 0 and options["bev_dsm"] and bev_add:              ### added 
...
def add_heat(n, costs, notes=False):   ##added 'notes' for no thermal storage
...
def build_new_wind(n,wind_op):
    ...
    """
    n.generators.p_nom_max[n.generators.carrier.str.contains('onwind')]= (
        n.generators.p_nom_max[n.generators.carrier.str.contains('onwind')]* wind_op )

    """
    #df_wind_new=pd.read_csv('wind_profiles/aggCFvoronoi137_1hr{}noInterpolation.csv'.format(wind_op),index_col=0,parse_dates=True)
    ...
    p_max_pu_=n.generators_t.p_max_pu[onwind]
    ...
    n.madd("Generator",
        ...
        carrier="onwind",
        p_nom_extendable=True,
        p_nom_max=(p_nom_max_*wind_op),
        marginal_cost=marginal_cost_,
        capital_cost=capital_cost_,
        efficiency=efficiency_,
        p_max_pu=p_max_pu_,     #df_wind for different wind data
        lifetime=lifetime_,
      )

    n.madd("Generator",              #added to stop infeasible model, 10 times the cost
        onwind,
        suffix=" extra",
        bus=bus_,
        carrier="onwind",
        p_nom_extendable=True,
        p_nom_max=(p_nom_max_*(1-wind_op)),
        marginal_cost=marginal_cost_,
        capital_cost=(capital_cost_*10),
        efficiency=efficiency_,
        p_max_pu=p_max_pu_,
        lifetime=lifetime_,
      )  
    ...
    ##offwind-ac :
    offwind_ac = n.generators.index[n.generators.carrier == "offwind-ac"]
    bus_ = n.generators.loc[offwind_ac, "bus"]
    p_nom_max_=n.generators.loc[offwind_ac, 'p_nom_max']
    p_max_pu_=n.generators_t.p_max_pu[offwind_ac]
    marginal_cost_=n.generators.loc[offwind_ac, 'marginal_cost']
    capital_cost_=n.generators.loc[offwind_ac, 'capital_cost']
    efficiency_=n.generators.loc[offwind_ac, 'efficiency']
    lifetime_=n.generators.loc[offwind_ac, 'lifetime']

    n.generators.drop(offwind_ac, inplace=True)
    ...
    ##offwind-dc :
    offwind_dc = n.generators.index[n.generators.carrier == "offwind-dc"]
    bus_ = n.generators.loc[offwind_dc, "bus"]
    p_nom_max_=n.generators.loc[offwind_dc, 'p_nom_max']
    p_max_pu_=n.generators_t.p_max_pu[offwind_dc]
    marginal_cost_=n.generators.loc[offwind_dc, 'marginal_cost']
    capital_cost_=n.generators.loc[offwind_dc, 'capital_cost']
    efficiency_=n.generators.loc[offwind_dc, 'efficiency']
    lifetime_=n.generators.loc[offwind_dc, 'lifetime']

    n.generators.drop(offwind_dc, inplace=True)
...
if __name__ == "__main__":
    ...
    if "T" in opts:
        if 'noEVbattery' in opts:     ### added for 'no EV battery' option
              add_land_transport(n, costs, bev_add=0)
        else :
              add_land_transport(n, costs)

    if "H" in opts:
      if 'notes' in opts:  
        add_heat(n, costs, notes=True)
      else:
        add_heat(n, costs)
    ...
    if options['electricity_distribution_grid']:
      ...
      elif "nohomebattery" in opts:                                                            ##  added for 'no home battery' option
        insert_electricity_distribution_grid(n, costs, distsolar=True, distbat=False, distribution_efficiency=dist_link_eff)      
      else  :
        insert_electricity_distribution_grid(n, costs, distribution_efficiency=dist_link_eff, rooftop_factor=rooftop_factor)
